...
public class LightsBehaviour : MonoBehaviour {
	...
	void OnHitSwitch (Vector3 switchPosition, GFGrid theGrid){
		...
		if(theGrid != _connectedGrid) {return;}
	...
	public void SwitchLights(){
		gameObject.GetComponent<Renderer>().material = _isOn ? _onMaterial : _offMaterial;
	}
	
	void OnMouseUpAsButton(){
		//we don't need an instance here, because this function is static
		SwitchManager.SendSignal(transform.position, _connectedGrid);
	}
}
