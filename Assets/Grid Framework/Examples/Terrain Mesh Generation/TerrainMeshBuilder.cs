...
public class TerrainMeshBuilder : MonoBehaviour {
	...
	/// <summary>Plain text file with (preferably) integer numbers for the height</summary>
	...
	/// <summary>Used for the rendering the plane.</summary>
	public Material _planeMaterial;
	/// <summary>Used for the wire frame rendering.</summary>
	...
	// the index is calculated by going through each row until the end and then
	// jumping to the beginning of the next row (rows and columns start at 1)
	...
	//handle mouse input by casting a ray from the cursor through the camera at
	//the mesh collider and returning the index of the nearest vertex
	int HandleMouseInput () {
		...
		// remember, the index is related to the grid coordinates
		return RowColumn2Index(Mathf.RoundToInt(-gp.z) + 1, Mathf.RoundToInt(gp.x) + 1);
	}
	...
	void RaiseVertex (int index, int amount) {
		...
		UpdateHeightString (); // update the height map in the GUI using the updated heights array
	}
	...
	// this is not necessarily the best way to achieve wireframe rendering, but
	// it gets the job done. For a real game you should look for a better
	// solution
	void OnRenderObject () {
	...
	void UpdateHeightString () {
		...
		for (int i = 1; i <= rows; i++) {
			for (int j = 1; j <= columns; j++) {
				heightString += " " + heights[RowColumn2Index(i, j)] + " "; // add the entries from the heights array
			}
